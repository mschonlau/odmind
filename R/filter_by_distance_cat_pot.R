...
#' @export
#'
filter_by_distance_cat_pot <- function(odm_object, distance_value, search_direction, target_df, filter_attribute, filter_value, ors_profile) {
  if (search_direction == "to_target") {
    x <- odm_object[["distance"]] %>%
      to_target_by_cat_distance_weighted(., odm_object, distance_value, target_df, filter_attribute, filter_value, ors_profile)
  }
  else {
    x <- odm_object[["distance"]] %>%
      from_target_by_cat_distance_weighted(., odm_object, distance_value, target_df, filter_attribute, filter_value, ors_profile)
  }
}
